        ...
        if subb=='':
            ...
            def enter_data_DB():
                ...
                if ENROLLMENT=='':
                    ...
                    try:
                        print(cursor.execute("sucess",Insert_data, VALUES))
                    except Exception as e:
                        print(e)
                    ENR_ENTRY.delete(first=0, last=22)
                    STUDENT_ENTRY.delete(first=0, last=22)'''
            ...
            def enter_data_DB():
                 ...
                 if ENROLLMENT == '' or STUDENT == '':
                        ...
                        VALUES = (str(ENROLLMENT), str(STUDENT), str(Date), str(time))
                        try:
                            print(cursor.execute(Insert_data, VALUES))
                        except Exception as e:
                            print(e)
                        ENR_ENTRY.delete(first=0, last=22)
                        STUDENT_ENTRY.delete(first=0, last=22)
            ...
            def create_csv():
                ...
                csv_name='C:/Users/DELL/Desktop/python projects/akdr/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            #def attf():
                #import subprocess
                #subprocess.Popen(r'explorer /select,"C:\Users\DELL\Downloads\akdr\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='snow')
                cs = 'C:/Users/DELL/Desktop/python projects/akdr/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:\Users\DELL\Desktop\python projects\akdr\Attendance\-------Check atttendance-------"')
...
def admin_panel():
    ...
    def open_directory():
        directory_path = r"C:\Users\DELL\Desktop\python projects\akdr\StudentDetails"
        file_path = os.path.join(directory_path, "StudentDetails.csv")
        if os.path.exists(file_path):  # Check if the file exists
            os.startfile(directory_path)  # Open the directory in the default file explorer
        else:
            print("File not found:", file_path)
    
    fill_a = tk.Button(win, text="Edit", fg="white", bg="blue2", width=15, height=1, activebackground="Red", font=('times', 15, ' bold '), command=open_directory)
    fill_a.place(x=650, y=250)

    fill_a = tk.Button(win, text="Delete", fg="white", bg="Red", width=15, height=1, activebackground="Red", font=('times', 15, ' bold '),command=open_directory)
    fill_a.place(x=650, y=300)
    ...
    def log_in():
        ...
        if username == 'admin' :
            if password == 'admin@123':
                ...
                ebth = tk.Button(window, text="Edit students",command=admin_panel,fg="black"  ,bg="cyan"  ,width=19 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
                ebth.place(x=990, y=410)

                cs = 'C:/Users/DELL/Desktop/python projects/akdr/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
